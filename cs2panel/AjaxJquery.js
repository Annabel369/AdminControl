        ...
        if (data.length === 0) {
            ...
            data.forEach(item => {
                ...
                if (type === 'admins') {
                    ...
                    const statusIcon = item.unmuted ? '<i class="fas fa-check-circle icon-unban"></i>' : '<i class="fas fa-volume-mute icon-ban"></i>';
                    innerHTML = `
                        <td class="status-cell">${statusIcon} ${statusText}</td>
                        <td>${item.steamid}</td>
                        <td>${item.reason}</td>
